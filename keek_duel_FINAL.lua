...
btnDirLeft.MouseButton1Click:Connect(function() 
...
btnDirRight.MouseButton1Click:Connect(function() 
...
btnAuto.MouseButton1Click:Connect(function() if not checkAuto() then return end; 
...
--// NEW L/R AUTO PLAY SYSTEM //
local autoPlayEnabled = false
local currentPoint = "L1"
...
local function setPosition(slot)
    local char = LocalPlayer.Character
    if char and char:FindFirstChild("HumanoidRootPart") then
        positions[slot] = char.HumanoidRootPart.CFrame
        print(slot.." set")
    end
end
...
RunService.Heartbeat:Connect(function()
    if autoPlayEnabled then
        local char = LocalPlayer.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
            local pos = positions[currentPoint]
            if pos then
