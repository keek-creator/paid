...
--// =========================
--// MODERN UI + STRETCH UPDATE
--// =========================

Config.StretchScreen = Config.StretchScreen or false
Config.StretchScale = Config.StretchScale or 1

local TweenService = game:GetService("TweenService")

local function ApplyModernUI(frame)
    ...
    pcall(function()
        ...
        if not frame:FindFirstChild("ModernCorner") then
            local corner = Instance.new("UICorner")
        ...
        if not frame:FindFirstChild("ModernStroke") then
            local stroke = Instance.new("UIStroke")
...
task.spawn(function()
    ...
    if main then
        ...
        local uiScale = main:FindFirstChild("CustomScale") or Instance.new("UIScale")
        uiScale.Name = "CustomScale"
        uiScale.Parent = main
        uiScale.Scale = Config.StretchScale
        ...
        local blur = game:GetService("Lighting"):FindFirstChild("EmoteGUIBlur")

        local function ToggleBlur(enabled)
            if enabled then
                if not blur then
                    blur = Instance.new("BlurEffect")
                    blur.Name = "EmoteGUIBlur"
                    blur.Size = 12
