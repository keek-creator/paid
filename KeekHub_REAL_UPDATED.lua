...
-- ===================== NEW MOBILE BUTTON SYSTEM =====================
local MobileButtons = {}
local MB_SIZE = 55
local savedPositions = {}

local function makeDrag(btn,key)
	local drag=false
	local start,inputStart
	...
	btn.InputEnded:Connect(function(i)
		if i.UserInputType==Enum.UserInputType.Touch or i.UserInputType==Enum.UserInputType.MouseButton1 then
			drag=false
			savedPositions[key]=btn.Position
		end
	end)
	...
	game:GetService("UserInputService").InputChanged:Connect(function(i)
		if drag and (i.UserInputType==Enum.UserInputType.Touch or i.UserInputType==Enum.UserInputType.MouseMovement) then
			...
			btn.Position=UDim2.new(start.X.Scale,start.X.Offset+d.X,start.Y.Scale,start.Y.Offset+d.Y)
		end
	end)
end

local function makeBtn(name,text,callback,pos)
	local b=Instance.new("TextButton")
	b.Size=UDim2.new(0,MB_SIZE,0,MB_SIZE)
	b.Position=pos
	b.Text=text
	...
	b.InputBegan:Connect(function()
		game:GetService("TweenService"):Create(b,TweenInfo.new(0.08),{Size=UDim2.new(0,MB_SIZE-6,0,MB_SIZE-6)}):Play()
	end)

	b.InputEnded:Connect(function()
		game:GetService("TweenService"):Create(b,TweenInfo.new(0.12,Enum.EasingStyle.Back),{Size=orig}):Play()
	end)
...
makeBtn("AutoLeft","L",function() autoLeftEnabled=not autoLeftEnabled if autoLeftEnabled then startAutoLeft() else stopAutoLeft() end end,UDim2.new(0,20,0.6,0))
makeBtn("AutoRight","R",function() autoRightEnabled=not autoRightEnabled if autoRightEnabled then startAutoRight() else stopAutoRight() end end,UDim2.new(0,90,0.6,0))
makeBtn("AutoBat","BAT",function() autoBatEnabled=not autoBatEnabled if autoBatEnabled then startBatAimbot() else stopBatAimbot() end end,UDim2.new(0,160,0.6,0))
makeBtn("Drop","DROP",runDrop,UDim2.new(0,20,0.7,0))
makeBtn("TP","TP",runTPDown,UDim2.new(0,90,0.7,0))
makeBtn("Lagger","LAG",function() laggerMode=not laggerMode end,UDim2.new(0,160,0.7,0))
