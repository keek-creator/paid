...
local cfg = cfg or {}
cfg.buttonSize = cfg.buttonSize or 70
cfg.mobilePositions = cfg.mobilePositions or {}
...
local function saveConfig()
	if writefile then
		...
		for name,btn in pairs(MobileButtons) do
			data.mobilePositions[name] = {
				x = btn.Position.X.Offset,
				y = btn.Position.Y.Offset
			}
		end

		writefile("KeekHubConfig.json", HttpService:JSONEncode(data))
	end
end
...
local function loadConfig()
	if readfile then
		local success,data = pcall(function()
			return HttpService:JSONDecode(readfile("KeekHubConfig.json"))
		end)

		if success and data then
			cfg.buttonSize = data.buttonSize or 70
			cfg.mobilePositions = data.mobilePositions or {}
		end
	end
end
...
local function makeBtn(name,text,defaultX,defaultY,callback)
	local size = cfg.buttonSize

	local pos = cfg.mobilePositions[name]
	local x = pos and pos.x or defaultX
	local y = pos and pos.y or defaultY
...
-- LOCK BUTTON
makeBtn("LOCK","🔓",20,120,function(self)
	mobileLocked = not mobileLocked
	self.Text = mobileLocked and "🔒" or "🔓"
end)

-- MAIN BUTTONS
makeBtn("DROP","DROP\nBR",20,200,function() if runDrop then task.spawn(runDrop) end end)
makeBtn("AUTOLEFT","AUTO\nLEFT",20,280,function() if startAutoLeft then startAutoLeft() end end)
makeBtn("AUTOBAT","AUTO\nBAT",20,360,function() if startBatAimbot then startBatAimbot() end end)

makeBtn("AUTORIGHT","AUTO\nRIGHT",120,200,function() if startAutoRight then startAutoRight() end end)
makeBtn("TP","TP\nDOWN",120,280,function() if runTPDown then task.spawn(runTPDown) end end)
makeBtn("CARRY","CARRY\nSPD",120,360,function() speedMode = not speedMode end)

makeBtn("BATC","BAT\nCTR",120,440,function() if startBatCounter then startBatCounter() end end)
makeBtn("LAG","LAG\nMODE",120,520,function() if startLaggerMode then startLaggerMode() end end)

print("KEEK MOBILE SAVED SYSTEM LOADED")
