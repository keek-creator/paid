...
-- ===================== KEEK FLOATING BUTTON SYSTEM =====================
local FloatingButtons = {}
local ButtonSize = 60
local FloatingLocked = true

local function createFloatingButton(gui,name,label,callback)
	...
	btn.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 and not FloatingLocked then
			dragging = true
	...
	game:GetService("UserInputService").InputChanged:Connect(function(input)
		if dragging and input.UserInputType == Enum.UserInputType.MouseMovement and not FloatingLocked then
	...
	FloatingButtons[name] = {Frame = btn}
	return btn
end

-- create buttons (after GUI exists)
task.spawn(function()
	repeat task.wait() until game:GetService("Players").LocalPlayer:FindFirstChild("PlayerGui")

	local gui = game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("KeekHubGUI") or game:GetService("CoreGui"):FindFirstChild("KeekHubGUI")
	if not gui then return end

	createFloatingButton(gui,"DROP","DROP",runDrop)

	createFloatingButton(gui,"AUTOLEFT","L",function()
		autoLeftEnabled = not autoLeftEnabled
		if autoLeftEnabled then startAutoLeft() else stopAutoLeft() end
	end)

	createFloatingButton(gui,"AUTORIGHT","R",function()
		autoRightEnabled = not autoRightEnabled
		if autoRightEnabled then startAutoRight() else stopAutoRight() end
	end)

	createFloatingButton(gui,"AUTOBAT","BAT",function()
		autoBatEnabled = not autoBatEnabled
		if autoBatEnabled then startBatAimbot() else stopBatAimbot() end
	end)

	createFloatingButton(gui,"TPDOWN","TP",runTPDown)

	createFloatingButton(gui,"CARRY","SPD",function()
		speedMode = not speedMode
	end)

	createFloatingButton(gui,"BATCOUNTER","CTR",function()
		if batCounterEnabled then
			stopBatCounter()
		else
			startBatCounter()
	...
	createFloatingButton(gui,"LAGGER","LAG",function()
		laggerMode = not laggerMode
		if laggerMode then startLaggerMode() else stopLaggerMode() end
	end)
	...
	lockBtn.Position = UDim2.new(0,10,0.8,0)
	lockBtn.BackgroundColor3 = Color3.fromRGB(25,25,30)
	lockBtn.Text = FloatingLocked and "🔒" or "🔓"
