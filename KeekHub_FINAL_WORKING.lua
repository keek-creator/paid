...
local function makeDraggable(frame)
	...
	UIS.InputChanged:Connect(function(input)
		if input == dragInput and dragging and not mobileLocked then
...
-- LOCK BUTTON
local lockBtn = makeBtn("🔓",20,120,function()
...
makeBtn("AUTO\nLEFT",20,270,function()
	...
	if autoBatEnabled then autoBatEnabled=false if autoBatSetVisual then autoBatSetVisual(false) end if stopBatAimbot then stopBatAimbot() end end
	autoLeftEnabled = not autoLeftEnabled
	if autoLeftSetVisual then autoLeftSetVisual(autoLeftEnabled) end
	if autoLeftEnabled and startAutoLeft then startAutoLeft() elseif stopAutoLeft then stopAutoLeft() end
...
makeBtn("AUTO\nBAT",20,340,function()
	...
	if autoRightEnabled then autoRightEnabled=false if autoRightSetVisual then autoRightSetVisual(false) end if stopAutoRight then stopAutoRight() end end
	autoBatEnabled = not autoBatEnabled
	if autoBatEnabled and startBatAimbot then startBatAimbot() elseif stopBatAimbot then stopBatAimbot() end
	if autoBatSetVisual then autoBatSetVisual(autoBatEnabled) end
	if saveConfig then saveConfig() end
end)

makeBtn("AUTO\nRIGHT",110,200,function()
	if autoLeftEnabled then autoLeftEnabled=false if autoLeftSetVisual then autoLeftSetVisual(false) end if stopAutoLeft then stopAutoLeft() end end
	if autoBatEnabled then autoBatEnabled=false if autoBatSetVisual then autoBatSetVisual(false) end if stopBatAimbot then stopBatAimbot() end end
	autoRightEnabled = not autoRightEnabled
	if autoRightSetVisual then autoRightSetVisual(autoRightEnabled) end
	if autoRightEnabled and startAutoRight then startAutoRight() elseif stopAutoRight then stopAutoRight() end
	if saveConfig then saveConfig() end
end)

makeBtn("TP\nDOWN",110,270,function() if runTPDown then task.spawn(runTPDown) end end)

makeBtn("CARRY\nSPEED",110,340,function()
	speedMode = not speedMode
	if modeValLbl then modeValLbl.Text = speedMode and "Carry" or "Normal" end
	if saveConfig then saveConfig() end
end)

makeBtn("BAT\nCOUNTER",110,410,function()
	batCounterEnabled = not batCounterEnabled
	if setBatCounterVisual then setBatCounterVisual(batCounterEnabled) end
	if batCounterEnabled and startBatCounter then startBatCounter() elseif stopBatCounter then stopBatCounter() end
	if saveConfig then saveConfig() end
end)

makeBtn("LAGGER\nMODE",110,480,function()
	laggerMode = not laggerMode
	if setLaggerVisual then setLaggerVisual(laggerMode) end
	if laggerMode and startLaggerMode then startLaggerMode() elseif stopLaggerMode then stopLaggerMode() end
	if saveConfig then saveConfig() end
end)

print("KEEK MOBILE FINAL LOADED")
-- ===================== END FIX =====================
